...
<script type="text/javascript">	
	...
	//单选按钮选触发时间（选中所有页面的case/取消选中所有页面的case）
	
	function selectAllPageCases(radio){
		var radioValue = $(radio).val();
		$.ajax({
				type:'post',
				url:'testcase!selectAllPageCases.action',
				data:"opFlag="+radioValue+"&testType=<s:property value='testType' />&testCode=<s:property value='testCode' />",
				success:function(result){
					if("success" != result){
	...
	//选择全部testcase时触发事件,包括勾选全部和取消勾选全部
	function selectAllTestCases(checkbox){
		...
		if(($(checkbox).prop('checked'))){//全选按钮未勾选时单击全选按钮
			...
			$("#table input[name=check]").each(function(){ //遍历table里的全部checkbox    $("input:checkbox[name='the checkbox name']:checked")  
			...
			<form id="frmSearch" name="frmSearch" action="testcase!list.action" method="post" namespace="/" >
				...
				<div id="formTable" class="rowClass">				
					<table style = "{border=0; cellpadding=0 ;cellspacing=0;}">
						<tr>
							<td><input type='radio' name='opFlag' value='0' style="width: 28px;" onclick="javascript:selectAllPageCases(this);" >Unselect</td>
						</tr>
						<tr>
							<td><input type='radio' name='opFlag' value='1' style="width: 28px;" onclick="javascript:selectAllPageCases(this); ">Select</td>
						</tr>
					</table>
						...
						<th width="10%">
							...
							<s:if test="selected==true">
								<input type="checkbox" name="checkAll" class="checkAll" checked="checked"
								       id="checkAll" onclick="javascript:selectAllTestCases(this);"/>Selected
							</s:if>
							<s:else>
